...
// 	t.Run("Success - List content types with project filter", func(t *testing.T) {
// 		// Create project
...
// 		projectID := project.ID
// 		database.DB.Create(&models.ContentType{Name: "Project Blog", Slug: "project-blog", ProjectID: &projectID})

// 		resp, err := testutils.MakeRequest(app, "GET", fmt.Sprintf("/content/types?project_id=%d", project.ID), nil, token)
// 		assert.NoError(t, err)
// 		assert.Equal(t, 200, resp.Code)
...
// 	// Create users
// 	owner := testutils.CreateTestUser(t, database.DB, "owner@test.com", "password", "admin")
// 	editor := testutils.CreateTestUser(t, database.DB, "editor_project@test.com", "password", "editor")
// 	viewer := testutils.CreateTestUser(t, database.DB, "viewer_project@test.com", "password", "viewer")

// 	ownerToken := testutils.GetAuthToken(t, owner.ID, owner.Role.Name)
// 	editorToken := testutils.GetAuthToken(t, editor.ID, editor.Role.Name)
...
// 		CreatedBy:     editor.ID,
// 		Status:        models.StatusDraft,
// 		Data:          datatypes.JSON([]byte(`{"title":"Project Entry"}`)),
// 	}
// 	database.DB.Create(entry)

// 	t.Run("Success - Project editor requests review", func(t *testing.T) {
// 		body := map[string]interface{}{
// 			"status":  "in_review",
// 			"comment": "Ready for review",
// 		}
...
// 	t.Run("Success - Project owner approves", func(t *testing.T) {
// 		// First, mark as ready for approval
// 		database.DB.Model(&models.ContentEntry{}).Where("id = ?", entry.ID).Update("status", models.StatusReadyForApproval)
...
// 		resp, err := testutils.MakeRequest(app, "POST", "/workflow/entries/"+fmt.Sprint(entry.ID)+"/status", body, ownerToken)
// 		assert.NoError(t, err)
// 		assert.Equal(t, 200, resp.Code)
// 	})

// 	t.Run("Error - Project viewer cannot change status", func(t *testing.T) {
// 		// Reset entry to draft
// 		database.DB.Model(&models.ContentEntry{}).Where("id = ?", entry.ID).Update("status", models.StatusDraft)
...
// 		resp, err := testutils.MakeRequest(app, "POST", "/workflow/entries/"+fmt.Sprint(entry.ID)+"/status", body, viewerToken)
// 		assert.NoError(t, err)
// 		// Viewer role maps to "viewer" which doesn't have permission, should return 400 or 403
// 		assert.True(t, resp.Code == 400 || resp.Code == 403)
// 	})

// 	t.Run("Error - Non-member cannot access project entry", func(t *testing.T) {
...
// 		resp, err := testutils.MakeRequest(app, "POST", "/workflow/entries/"+fmt.Sprint(entry.ID)+"/status", body, nonMemberToken)
// 		assert.NoError(t, err)
// 		assert.Equal(t, 400, resp.Code) // Should fail - not a member
// 	})
// }

// func TestProjectContentEntryCreation(t *testing.T) {
// 	app := testutils.SetupTestApp(t)

// 	admin := testutils.CreateTestUser(t, database.DB, "admin_project@test.com", "password", "admin")
// 	token := testutils.GetAuthToken(t, admin.ID, admin.Role.Name)
...
// 	t.Run("Success - Create entry for project", func(t *testing.T) {
// 		fields := map[string]string{
// 			"title": "Project Article Title",
// 		}

// 		resp, err := testutils.MakeMultipartRequest(app, "POST", fmt.Sprintf("/content/%d/entries?project_id=%d", ct.ID, project.ID), fields, token)
// 		assert.NoError(t, err)
// 		assert.Equal(t, 201, resp.Code)
...
// 	t.Run("Error - Non-member cannot create entry", func(t *testing.T) {
// 		nonMember := testutils.CreateTestUser(t, database.DB, "nonmember2@test.com", "password", "editor")
// 		nonMemberToken := testutils.GetAuthToken(t, nonMember.ID, nonMember.Role.Name)
...
// 		resp, err := testutils.MakeMultipartRequest(app, "POST", fmt.Sprintf("/content/%d/entries?project_id=%d", ct.ID, project.ID), fields, nonMemberToken)
// 		assert.NoError(t, err)
